    ...
    if (isNone()) {
      if (checkemail()) {
        if (localStorage.user) {
          ...
          var k = 0; // 循环取出，可用其他方法代理，数据量多的情况下，不建议使用for循环
          ...
          for (var e in arr) {
            ...
            if ($('#loginName').val().trim() == arr[e].loginName) {
              if ($('#loginPsd').val().trim() == arr[e].loginPsd) {
                alert('登录成功'); // 成功后清除用户名和密码

                clear();
                k = 0;
                window.location.href = "index.html"; // 成功之后对整个登录页面ID为web的部分重新换为成功的标识（也可以选择跳转到成功页面）

                $("#web").html("<span style='color: blue;'>登录成功【success】</span>");
                return;
              } else {
                alert('密码错误'); // 失败后清除用户名和密码
        ...
        if (a[0].length > 0) {
          if (a[1].length > 0) {
            ...
            if (b[0].length > 0) {
              ...
              } else {// document.getElementById("message").innerHTML = "<font color='red'>请输入正确的邮箱格式!!</font>";
              }
            } else {// document.getElementById("message").innerHTML = "<font color='red'>请输入正确的邮箱格式!!</font>";
              }
          } else {// document.getElementById("message").innerHTML = "<font color='red'>请输入正确的邮箱格式!!</font>";
